const Discord = require('discord.js');  //DeRx#2665
const client = new Discord.Client();  //DeRx#2665
const moment = require('moment');
const zalgo = require('zalgolize');  //DeRx#2665
const math = require('math-expression-evaluator');   //DeRx#2665 
const figlet = require('figlet');   //DeRx#2665
const fs = require('fs');  //DeRx#2665
const ms = require('ms');  //DeRx#2665
const prefix = '-'

client.on('message', msg => {  //DeRx#2665
    if (msg.content === 'باك') {  //DeRx#2665
      msg.reply('** :wink: وِلِـكُمِـ ﻧَوِرُتْ   :sparkling_heart:**');  //DeRx#2665
    }
  });

client.on('message', msg => {  //DeRx#2665
    if (msg.content === 'ويسو') {  //DeRx#2665
      msg.reply('**ويسو مات**');  //DeRx#2665
  });


client.on('message', msg => {  //DeRx#2665
    if (msg.content === 'احبك') {  //DeRx#2665
      msg.reply('**لاتحتك يا اخوي لو سمحت**');  //DeRx#2665
    }
  });
  ...
  client.on('message', msg => {  //DeRx#2665
    if (msg.content === 'احبك') {  //DeRx#2665
      msg.reply('**حبك برص يا سافل**');  //DeRx#2665
    }
  });
   ...
   client.on('message', msg => {  //DeRx#2665
    if (msg.content === 'هلا') {  //DeRx#2665
      msg.reply('**هلا بيك :heart: **');  //DeRx#2665
    }
  });
  ...
  client.on("message", message => {  //DeRx#2665
    if(message.content.startsWith(prefix + 'v2min')) {  //DeRx#2665
     let args = message.content.split(" ").slice(1);  //DeRx#2665
       var nam = args.join(' ');
    
      if(!message.member.hasPermission('ADMINISTRATOR')) return    message.channel.send('`ADMINISTRATOR` للأسف هذه الخاصية تحتاج الى ').then(msg => msg.delete(6000))
      if (!nam) return message.channel.send(`<@${message.author.id}> يجب عليك ادخال اسم`).then(msg => msg.delete(10000))  //DeRx#2665
      message.guild.createChannel(nam, 'voice').then(c => setTimeout(() => c.delete(), 120000))   //DeRx#2665
...
client.on("message", message => {    //DeRx#2665
    if(message.content.startsWith(prefix + "emoji")) {   //DeRx#2665
        if(message.author.bot) return;  //DeRx#2665
        var emojiid =  message.content.split(" ").slice(1).join(" ")   //DeRx#2665
        console.log(emojiid) 
        if(emojiid.length < "18" || emojiid.length > "18" || isNaN(emojiid)) return  message.channel.send(`- Usage  //WESO#0001
${prefix}emoji <EmojiID>`); 
        else    
        message.channel.send("This is the emoji that you requested:-",  //DeRx#2665
          { 
            files: [`https://cdn.discordapp.com/emojis/${emojiid}.png`]  //DeRx#2665
          }) 
        }  
})   
 ...
 client.on("message", message => {  //DeRx#2665
  if (message.channel.type === "dm") {    //DeRx#2665

      message.channel.startTyping();    //DeRx#2665
      setTimeout(() => {    //DeRx#2665
        message.channel.stopTyping();    //DeRx#2665
      }, Math.random() * (1 - 3) + 1 * 1000);  //DeRx#2665
   
  }  
});  

const clans = {};  //DeRx#2665
const system = {};  //DeRx#2665
const level = {};  //DeRx#2665
 
 
client.on('message',async message => {  //DeRx#2665
  if(message.author.bot) return;  //
  if(message.channel.type === 'dm') return;  //DeRx#2665
 
  let args = message.content.split(' ');  //DeRx#2665
  let random = Math.floor(Math.random() * 5) + 2;  //DeRx#2665
  let author = message.author;
 
  let xpLeft;  //DeRx#2665
  let nameClan;  //DeRx#2665
  let membersClan = [];  //DeRx#2665
  let levelClan = 0;  //
  if(!system[author.id]) system[author.id] = {clan: 'None',joinedAt: new Date().toLocaleString() ,clanLevel: 0};  //DeRx#2665
 
  if(!level[author.id]) level[author.id] = {level: 1, xp: 1};  //DeRx#2665
 
 
  level[author.id].xp += (+random);  //DeRx#2665
  if(level[author.id].xp >= 300) {  //DeRx#2665
    if(level[author.id].xp > 300) xpLeft = level[author.id].xp - 300;  //DeRx#2665
    level[author.id] = {  //DeRx#2665
      level: level[author.id].level + 1,  //DeRx#2665
      xp: xpLeft  //DeRx#2665
    };
 
  }
  if(message.content.startsWith(prefix + "clan")) {  //DeRx#2665
    if(message.content.split(' ')[0] !== `${prefix}clan`) return;  //DeRx#2665
 
    if(!args[1] || args[1] && args[1] === 'info') {  //DeRx#2665
  let embed = new Discord.RichEmbed()
  .setAuthor('الكلانات', message.author.avatarURL)  //DeRx#2665
  ...
  .setFooter(message.author.username, message.author.avatarURL);  //DeRx#2665
  message.channel.send(embed);
}
 
    if(args[1] && args[1] === 'create') {  //DeRx#2665
      ...
      let awaited = await message.channel.awaitMessages(r => r.author.id === message.author.id, { max: 1, time: 20000, errors: ['time']}).then(collected => {
        ...
        nameClan = collected.first().content;  //DeRx#2665
      });
 
      m = await m.edit('**# جارى عمل الكلان**');  //DeRx#2665
      awaited = await setTimeout(async() => {  //DeRx#2665
        let membersArray = {
          nameClan: {
            array: []
          }
        };
        let members = membersArray.nameClan.array;  //DeRx#2665
        members.push(message.author.id);
        clans[nameClan] = {
          name: nameClan,
          createdAt: new Date().toLocaleString(),  //DeRx#2665
          level: levelClan,
          creator: message.author.id,  //DeRx#2665
          members: members,  //DeRx#2665
          applylist: [],  //DeRx#2665
          admins: []  //DeRx#2665  //DeRx#2665
        };
 
        system[author.id] = {  //DeRx#2665
          clan: nameClan,  //DeRx#2665
          joinedAt: new Date().toLocaleString(),  //DeRx#2665
          clanLevel: 0,  //WESO#0001
          creator: message.author.id  //DeRx#2665
        };
 
        m = await m.edit('**# تم عمل الكلان بنجاح**');  //DeRx#2665
      }, 2300);
 
    }
    if(args[1] && args[1] === 'invite') {  //DeRx#2665
      if(!system[author.id]) return message.channel.send("**# أنت لست بكلان**");  //DeRx#2665
      let clan = system[author.id].clan;
      if(system[author.id].clan === 'None') return message.channel.send('**# أنت لست بكلان**');  //DeRx#2665
      ...
      let mention = message.mentions.users.first();  //DeRx#2665
      if(!mention) return message.channel.send('**# منشن شخص لدعوته للكلان**');  //DeRx#2665
      if(clans[clan].members.includes(mention.id)) return message.channel.send("**# هذا العضو بالكلان بالفعل**");  //DeRx#2665
      if(clans[clan].members.length === 10) return message.channel.send("**# هذا الكلان وصل للحد الاقصى من الاعضاء يمكنك**");  //DeRx#2665
   //  //  //  //  //
      ...
      let awaiting = await message.channel.awaitMessages(r => r.author.id === mention.id, {max: 1, time: 20000, errors:['time']}).then(collected => {
        collected.first().delete().catch();  //DeRx#2665
        if(collected.first().content === 'نعم') {  //DeRx#2665
          clans[clan].members.push(mention.id);  //DeRx#2665
 
          system[author.id].members += 1;  //DeRx#2665
 
 
          system[mention.id] = {  //DeRx#2665
            clan: clan,
            joinedAt: new Date().toLocaleString(),  //DeRx#2665
            creator: clans[clan].creator  //DeRx#2665
          };
 
          message.channel.send(`**${message.author} # تم قبول الدعوة**`);  //DeRx#2665
        }  //
        if(collected.first().content === 'لا') {  //DeRx#2665
          message.channel.send(`**${message.author} # تم رفض الدعوة**`);  //DeRx#2665
        } else if(collected.first().content !== 'نعم' && collected.first().content !== 'لا'){  //DeRx#2665
          return message.channel.send('**# يجب عليك كتابة `نعم` أو `لا`**');  //DeRx#2665
        }  //DeRx#2665
      });  //DeRx#2665
    }  //DeRx#2665
    if(args[1] && args[1] === 'stats') {
      if(system[author.id].clan === 'None') return message.channel.send('**# يجب ان تكون بكلان لأستخدام هذا الأمر**');  //DeRx#2665
      let clan = system[author.id].clan;  //DeRx#2665
      let embed = new Discord.RichEmbed()  //DeRx#2665
        .setAuthor(`${message.author.username} || الكلانات`, message.author.avatarURL)  //DeRx#2665
        .setDescription(`الكلان || \`${clan.toString()}\``)  //DeRx#2665
        embed.addField('» اسم الكلان', clan, true)  //DeRx#2665
        embed.addField('» تاريخ عمل الكلان', clans[clan].createdAt, true);  //DeRx#2665
        embed.addField('» تاريخ دخول الكلان', system[author.id].joinedAt, true)  //DeRx#2665
        embed.addField('» صاحب الكلان', `<@${clans[clan].creator}>`, true);  //DeRx#2665
        embed.addField('» لفل الكلان', clans[clan].level, true);  //DeRx#2665
        embed.addField('» عدد اعضاء الكلان', clans[clan].members.length, true);  //DeRx#2665
        embed.addField('» عدد التقديمات للكلان', clans[clan].applylist.length, true);  //DeRx#2665
        embed.addField('» عدد الادمنية بالكلان', clans[clan].admins.length, true);  //DeRx#2665
        embed.addField('» اعضاء الكلان', `${prefix}clan list || يظهرلك رسالة بها اعضاء الكلان`);  //DeRx#2665
      message.channel.send(embed);  //DeRx#2665
 
    }
    if(args[1] && args[1] === 'join') {  //DeRx#2665  //DeRx#2665
    let clanName = message.content.split(' ').slice(2).join(" ");  //DeRx#2665
    if(system[author.id].clan !== 'None') return message.channel.send("**# يجب أن لا تكون بكلان**");  //DeRx#2665
    if(!args[2]) return message.channel.send("**# يجب عليك كتابة اسم الكلان**");  //DeRx#2665
    if(!clans[clanName]) return message.channel.send("**# هذا الكلان غير موجود**");  //DeRx#2665
    if(clans[clanName].applylist.includes(message.author.id)) return message.channel.send("**# لقد قدمت على دخول هذا الكلان مسبقا");  //DeRx#2665
   //DeRx#2665  //DeRx#2665  //DeRx#2665
    clans[clanName].applylist.push(message.author.id);  //DeRx#2665
    message.channel.send("**# لقد تم التقديم على دخول الكلان , سيتم الرد عليك من قبل احد ادارة الكلان**");  //DeRx#2665
  }
    if(args[1] && args[1] === 'accept') {  //DeRx#2665
      let mention = message.mentions.users.first();
      if(system[author.id].clan === 'None') return message.channel.send("**# يجب عليك ان تكون بكلان لأستخدام هذا الأمر**");  //DeRx#2665
      ...
      if(!system[mention.id]) system[mention.id] = {clan: 'None',joinedAt: new Date().toLocaleString() ,clanLevel: 0};  //DeRx#2665
      ...
      clans[system[author.id].clan].applylist.shift(mention.id);  //DeRx#2665
      clans[system[author.id].clan].members.push(mention.id);  //DeRx#2665
      let clan = system[author.id].clan;  //DeRx#2665
   ...
   //DeRx#2665
      ...
      mention.send(`**# \`${system[author.id].clan}\`, لقد تم قبولك بالكلان**`).catch();  //DeRx#2665
      message.channel.send(`**# \`${mention.username}\`, لقد تم قبول الشخص ودخوله للكلان**`);  //DeRx#2665
    }  //DeRx#2665
    if(args[1] && args[1] === 'decline') {  //DeRx#2665
      let mention = message.mentions.users.first();  //DeRx#2665
      ...
      system[mention.id] = {  //DeRx#2665
        clan: clans[system[author.id].clan],  //DeRx#2665
        joinedAt: new Date().toLocaleString(),
        clanLevel: 0  //DeRx#2665
      };
 
 
      mention.send(`**# \`${system[author.id].clan}\`, لقد تم رفض دخولك للكلان**`).catch();  //DeRx#2665
      message.channel.send(`**# \`${mention.username}\`, لقد تم رفض دخول الشخص للكلان**`);  //DeRx#2665
   //WESO#0001
    }
    if(args[1] && args[1] === 'promote') {
      ...
      if(system[mention.id].clan === 'None') return message.channel.send("**# هذا الشخص ليس بكلان**");  //DeRx#2665
      if(!clans[system[author.id].clan].members.includes(mention.id)) return message.channel.send("**# هذا الشخص ليس بالكلان**");  //DeRx#2665
      if(clans[system[author.id].clan].admins.includes(mention.id)) return message.channel.send("**# هذا العضو لديه ادمن بالفعل**");  //DeRx#2665
      if(mention.id === message.author.id) return message.channel.send("**# لا يمكنك اعطاء نفسك ترقية**");  //DeRx#2665
 
      clans[system[author.id].clan].admins.push(mention.id);  //DeRx#2665
 
 
      mention.send(`**# \`${system[author.id].clan}\`, لقد تم ترقيتك الى ادمن**`).catch();  //DeRx#2665
      message.channel.send(`**# \`${mention.username}\`, لقد تم ترقية العضو الى رتبة ادمن**`);  //DeRx#2665
    }  //DeRx#2665
    if(args[1] && args[1] === 'demote') {  //DeRx#2665
      let mention = message.mentions.users.first();  //DeRx#2665
      if(system[author.id].clan === 'None') return message.channel.send("**# يجب ان تكون بكلان لأستخدام هذا الأمر**");  //DeRx#2665
      if(clans[system[author.id].clan].creator !== message.author.id) return message.channel.send("**# هذا الأمر لضاحب الكلان فقط**");  //DeRx#2665
      if(!mention) return message.channel.send("**# يجب عليك منشنة عضو بالكلان لأعطائه الترقية**");  //DeRx#2665
      if(!system[mention.id]) system[mention.id] = {clan: 'None',joinedAt: new Date().toLocaleString() ,clanLevel: 0};  //DeRx#2665
   //DeRx#2665
      if(system[mention.id].clan === 'None') return message.channel.send("**# هذا الشخص ليس بكلان**");  //DeRx#2665
      if(!clans[system[author.id].clan].members.includes(mention.id)) return message.channel.send("**# هذا الشخص ليس بالكلان**");  //DeRx#2665
      if(!clans[system[author.id].clan].admins.includes(mention.id)) return message.channel.send("**# هذا الشخص ليس ادمن بالكلان**");  //DeRx#2665
      if(mention.id === message.author.id) return message.channel.send("**# لا يمكنك اعطاء نفسك ترقية**");  //DeRx#2665
   //DeRx#2665
      clans[system[author.id].clan].admins.shift(mention.id);  //DeRx#2665
...
client.on('message', message => {
  var prefix = "-"
        if(message.content.startsWith(prefix + '-ip')) {
            let args = message.content.split(' ').slice(1).join(' ');
            if (!args) return message.channel.send("**ip server. **");
            var link = (`    51.89.173.35:32054  	connect 51.89.173.35:32054	/${args}`);
